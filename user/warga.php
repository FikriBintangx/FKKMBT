...
if (!empty($blok_filter)) {
    // Check if it's a combined code like "A1" (Letter + Number)
    if (preg_match('/^([A-Z])(\d+)$/', $blok_filter, $matches)) {
        $b = $matches[1]; // A
        $n = $matches[2]; // 1
        $sql .= " AND blok = '" . $conn->real_escape_string($b) . "' AND no_rumah = '" . $conn->real_escape_string($n) . "'";
    } else {
        // Just a block letter like "A"
