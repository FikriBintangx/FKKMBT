...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['action'])) {
    if ($_POST['action'] == 'verify') {
        ...
        $stmt = $conn->prepare("UPDATE pembayaran_iuran SET status = ?, catatan_admin = ? WHERE id = ?");
        $stmt->bind_param("ssi", $status, $catatan, $id);
        $stmt->execute();
        setFlash('success', 'Status iuran berhasil diperbarui!');
        redirect('admin/iuran.php');
    }
    
    if ($_POST['action'] == 'add_master') {
        $nama = $_POST['nama_iuran'];
        $nom = $_POST['nominal'];
        $jt = $_POST['jatuh_tempo'];
        $conn->query("INSERT INTO iuran_master (nama_iuran, nominal, jatuh_tempo) VALUES ('$nama', '$nom', '$jt')");
        setFlash('success', 'Master iuran baru berhasil ditambahkan!');
        redirect('admin/iuran.php');
    }
}

// 1. Fetch ALL Pending Data into Array first (to separate Lists & Modals)
$pending_query = $conn->query("SELECT p.*, w.nama_lengkap, w.blok, w.no_rumah, m.nama_iuran, m.nominal 
...
$history = $conn->query("SELECT p.*, w.nama_lengkap, w.blok, w.no_rumah, m.nama_iuran, m.nominal, p.tgl_bayar 
                        FROM pembayaran_iuran p 
                        JOIN warga w ON p.warga_id = w.id 
                        JOIN iuran_master m ON p.iuran_id = m.id 
                        WHERE p.status != 'pending' 
                        ORDER BY p.tgl_bayar DESC LIMIT 10");

// Get Active Masters
$masters = $conn->query("SELECT * FROM iuran_master ORDER BY jatuh_tempo DESC");
?>
<!DOCTYPE html>
<html lang="id">
<head>
...
<body class="bg-light">
    ...
    <div class="container py-4">
        <div class="d-flex justify-content-between align-items-center mb-4">
            ...
            <button class="btn btn-primary-custom" data-bs-toggle="modal" data-bs-target="#addMaster">
                        ...
                        <div class="d-flex flex-column gap-3">
                            ...
                            <div class="p-3 bg-white border rounded-3 hover-shadow transition-all d-flex align-items-center justify-content-between flex-wrap gap-3">
                                ...
                                <div class="d-flex align-items-center gap-3">
                                    ...
                                    <!-- Use distinct ID for modal target -->
                            ...
                            <div class="d-flex gap-3 align-items-start border-bottom pb-3">
                                ...
                                <div>
                                    ...
                                    <small class="text-muted" style="font-size: 0.75rem;">
                                        <!-- FIX: Used tgl_bayar instead of last_update -->
    ...
    <!-- MODALS SECTION (Moved outside main layout to fix z-index/click issues) -->
    ...
    <script>
        function showImage(src) {
            ...
            const fullPath = '../assets/images/bukti_iuran/' + src;
