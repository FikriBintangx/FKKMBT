...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    if (isset($_POST['action'])) {
        ...
        if ($_POST['action'] == 'edit') {
            ...
            $conn->query("UPDATE warga SET nama_lengkap = '$nama', blok = '$blok', no_rumah = '$no_rumah', no_hp = '$hp' WHERE id = '$id'");
...
if (!empty($blok_filter)) {
    $sql .= " AND w.blok = '" . $conn->real_escape_string($blok_filter) . "'";
}
if (!empty($search)) {
                ...
                <table class="table table-hover align-middle">
                    ...
                    <tbody>
                        ...
                        <tr>
                            ...
                            <td>
                                <?php if($row['foto']): ?>
                                    <img src="../assets/images/warga/<?= $row['foto'] ?>" class="rounded-circle" width="32" height="32" style="object-fit:cover; border: 2px solid #fff; box-shadow: 0 2px 5px rgba(0,0,0,0.1);">
                                <?php else: ?>
                                    <span class="text-muted small">-</span>
                                <?php endif; ?>
                            </td>
                            <td class="text-end px-4">
                                <button class="btn btn-sm btn-light text-primary me-2" data-bs-toggle="modal" data-bs-target="#editModal<?= $row['id'] ?>"><i class="bi bi-pencil-fill"></i></button>
                                <button class="btn btn-sm btn-light text-danger" data-bs-toggle="modal" data-bs-target="#deleteModal<?= $row['id'] ?>"><i class="bi bi-trash-fill"></i></button>
                            </td>
                        </tr>
                                    ...
                                    <div class="modal-body">
                                        ...
                                        <div class="row g-2 mb-3">
                                            <div class="col-6">
                                                ...
                                                <input type="text" name="blok" class="form-control" value="<?= $row['blok'] ?>" required>
                                            </div>
                                            <div class="col-6">
                                                <label class="form-label small fw-bold">No Rumah</label>
                                                <input type="text" name="no_rumah" class="form-control" value="<?= $row['no_rumah'] ?>" required>
                                            </div>
                                        </div>
                                        <div class="mb-3">
                                        ...
                                        <div class="mb-3 pt-3 border-top">
                                            ...
                                            <input type="password" name="password" class="form-control" placeholder="Isi hanya jika ingin ubah password">
                                        </div>
                                    </div>
        ...
        <form class="modal-content" method="POST">
            ...
            <div class="modal-body">
                ...
                <div class="row g-2 mb-3">
                    <div class="col-6">
                        <label class="form-label small fw-bold">Blok</label>
                        <input type="text" name="blok" class="form-control" required placeholder="A">
                    </div>
                    <div class="col-6">
                        <label class="form-label small fw-bold">No Rumah</label>
