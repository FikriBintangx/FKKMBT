...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if ($action == 'create') {
        ...
        if (isset($_POST['username']) && !empty(trim($_POST['username']))) {
            ...
            if ($check->num_rows > 0) {
                // If manual username provided and exists, we MUST error or append. 
                // User asked "jangan ada nomor", so better error out or let them know.
                // For now, to be safe, we will just error/append only if absolutely necessary 
                // but user said "jangan ada 90". 
                // Strategy: If Taken -> Error.
                $error_msg = "Username '$username' sudah dipakai! Silakan ganti yang lain.";
                // Should probably stop execution here or flag error.
                // But since structure is complex, let's just fall back to valid logic:
                // If error, we can't stop easily inside this if block without refactoring.
                // Let's rely on auto-append check ONLY if it was auto-generated.
                // BUT, to strictly follow "no numbers", we should just try to use it.
                // If it fails (duplicate entry) MySQL will error. 
                // Let's add simple check:
                 echo "<script>alert('Username sudah ada! Ganti yang lain.'); window.history.back();</script>";
                 exit;
            }
        } else {
            // Auto-generate from nama (lowercase, no spaces)
            $username = strtolower(str_replace(' ', '', $nama));
            
            // Check if username exists, add number if needed (Fallback for auto-gen only)
            $check = $conn->query("SELECT id FROM users WHERE username = '$username'");
            if ($check->num_rows > 0) {
                 // User said "jangan ada 90". Maybe they prefer no number?
                 // But we have to handle duplicates.
                 // We will append a cleaner number logic or just leave it since they likely will use manual input now.
                 $username .= rand(10, 99);
            }
        }
        ...
        $hashed = $password; // Store as Plain Text per user request to see passwords
    ...
    elseif ($action == 'update') {
        ...
        if (isset($_POST['password']) && !empty($_POST['password'])) {
             // We need to fetch user_id first
             $uid_q = $conn->query("SELECT user_id FROM warga WHERE id='$warga_id'")->fetch_assoc();
             if ($uid_q) {
                 $uid = $uid_q['user_id'];
                 // Update Username if changed
                 if (!empty($username)) {
                     // Check if new username available (exclude current user)
                     $chk = $conn->query("SELECT id FROM users WHERE username='$username' AND id != '$uid'");
                     if ($chk->num_rows == 0) {
                 ...
                 $new_pass = $_POST['password']; // Store Check as Plain Text
...
// Get all warga
$warga_list = $conn->query("SELECT w.*, u.username, u.password as raw_password FROM warga w JOIN users u ON w.user_id = u.id ORDER BY w.blok, w.no_rumah");
?>
<!DOCTYPE html>
<html lang="id">
<head>
...
<body>
            ...
            <div class="modal-content border-0 shadow-lg" style="border-radius: 20px;">
                ...
                <form method="POST">
                    ...
                    <div class="modal-body p-4">
                        ...
                        <div class="mb-3">
                            <label class="form-label fw-semibold">Username (Opsional, untuk login)</label>
                            <input type="text" name="username" class="form-control" placeholder="Biarkan kosong untuk auto-generate dari nama">
                        </div>
                        <div class="mb-3">
    ...
    <script>
        ...
        function editWarga(data) {
            ...
            // Load Existing Password (if not hashed)
            // If raw_password starts with $2y$, it's likely a hash -> keep empty
            // If it's plain text -> show it so admin can see/edit
