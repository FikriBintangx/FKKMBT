...
<body class="bg-light">
                    ...
                    <div class="activity-img-wrapper">
                            ...
                            <div id="<?= $carouselId ?>" class="carousel slide" data-bs-ride="carousel">
                                <div class="carousel-inner">
                                    <?php foreach($media as $idx => $item): 
                                        $local_file_path = '../assets/images/kegiatan/' . $item['file'];
                                        $file_exists = file_exists($local_file_path);
                                        $display_src = $file_exists ? $local_file_path : 'https://via.placeholder.com/800x600/134e4a/ffffff?text=File+Tidak+Ditemukan';
                                    ?>
                                    <div class="carousel-item <?= $idx == 0 ? 'active' : '' ?>">
                                        <?php if($item['tipe_file'] == 'video'): ?>
                                            <video src="<?= $display_src ?>" class="activity-img bg-dark" controls></video>
                                        <?php else: ?>
                                            <img src="<?= $display_src ?>" class="activity-img d-block w-100" alt="Dokumentasi">
                                        <?php endif; ?>
                                    </div>
                                    <?php endforeach; ?>
                                </div>
                        ...
                        <?php elseif(count($media) == 1): ?>
                            ...
                            <?php if($item['tipe_file'] == 'video'): ?>
                                <video src="<?= $display_src ?>" class="activity-img bg-dark" controls></video>
                            <?php else: ?>
                                <img src="<?= $display_src ?>" class="activity-img" alt="Dokumentasi">
                            <?php endif; ?>

                        <?php else: ?>
                            <!-- PLACEHOLDER IF NO IMAGE -->
                            <img src="https://via.placeholder.com/800x600/134e4a/ffffff?text=Belum+Ada+Dokumentasi" class="activity-img grayscale">
                        <?php endif; ?>
                    </div>
    ...
    <script>
        function showDetail(btn) {
            ...
            let mediaHtml = '';
            // NOTE: Due to client-side JS limitations, we cannot easily check file_exists here for the modal.
            // Ideally we should pass a 'valid_url' property from PHP. For now, we will assume if it failed in the main view, it will fail here 
            // but we can at least use the onerror event to fallback.
            
            const fallbackImg = "https://via.placeholder.com/800x600/134e4a/ffffff?text=Image+Not+Found";

            if(media.length > 0) {
                if(media.length === 1) {
                    if(media[0].tipe_file === 'video') {
                        mediaHtml = `<video src="../assets/images/kegiatan/${media[0].file}" class="w-100 h-100 object-fit-contain" controls onerror="this.style.display='none'; this.parentNode.innerHTML='<img src=\\'${fallbackImg}\\' class=\\'w-100 h-100 object-fit-cover\\'>'"></video>`;
                    } else {
                        mediaHtml = `<img src="../assets/images/kegiatan/${media[0].file}" class="w-100 h-100 object-fit-cover" onerror="this.src='${fallbackImg}'">`;
                    }
                } else {
                    mediaHtml = `
                    <div id="carouselModal" class="carousel slide" data-bs-ride="carousel" style="width:100%; height:100%;">
                        <div class="carousel-inner h-100">
                            ...
                            <div class="carousel-item h-100 ${i === 0 ? 'active' : ''}">
                                ${m.tipe_file === 'video' 
                                    ? `<video src="../assets/images/kegiatan/${m.file}" class="d-block w-100 h-100 object-fit-contain" controls onerror="this.style.display='none'; this.parentNode.innerHTML='<img src=\\'${fallbackImg}\\' class=\\'w-100 h-100 object-fit-cover\\'>'"></video>` 
                                    : `<img src="../assets/images/kegiatan/${m.file}" class="d-block w-100 h-100 object-fit-cover" onerror="this.src='${fallbackImg}'">`}
                            </div>
                            `).join('')}
                        </div>
                ...
                mediaHtml = `<img src="https://via.placeholder.com/800x600/134e4a/ffffff?text=Belum+Ada+Dokumentasi" class="w-100 h-100 object-fit-cover">`;
            }
